...
with open('subscriptions.json', 'r') as f:  
    data = json.load(f)
start_date = input("Please enter the start date in the format YYYY-MM-DDTHH:MM:SSZ: ")  
end_date = input("Please enter the end date in the format YYYY-MM-DDTHH:MM:SSZ: ")  
...
for item in data:  
    ...
    """Handling warning and critical alerts"""

    ideal=budget/30
    print("ideal - ",ideal)
    print("usage - ", usage)
    if (0.9*ideal)>usage>(0.8*ideal):
        # alert_mail.send_mail(os.path.join("plots/", f"{subs}.png"),"WARNING!!!")
        image_paths_w.append(os.path.join("plots/", f"{subs}.png"))
        print("WARNING!!!")
    elif usage>(0.9*ideal):
        # alert_mail.send_mail(os.path.join("plots/", f"{subs}.png"),"CRITICAL!!!")
        image_paths_c.append(os.path.join("plots/", f"{subs}.png"))
        print("CRITICAL!!!")
...
if len(image_paths_w)>0:
    alert_mail.send_mail(image_paths_w,"WARNING!!!")
if len(image_paths_c)>0:    
    alert_mail.send_mail(image_paths_c,"CRITICAL!!!")
